...
{
    public class DynamoDeviceCodeStore<TCode> : IDeviceCodeStore<TCode>
        where TCode : DynamoIdentityDeviceCode
    {
        ...
        public async Task<TCode> CreateAsync(string application, string scope, string userCode, string deviceCode, DateTimeOffset expiresAt, CancellationToken cancellationToken)
        {
            var code = (TCode)Activator.CreateInstance(typeof(TCode));
            
            code.Application = application;
            code.Scopes = scope.Split(' ').Where(s => !string.IsNullOrWhiteSpace(s)).ToList();
        ...
        public Task<string> GetSubjectAsync(TCode code, CancellationToken cancellationToken)
        {
        ...
        public Task<string> GetScopeAsync(TCode code, CancellationToken cancellationToken)
        {
            return Task.FromResult(string.Join(" ", code.Scopes));
        }

        public Task Consume(TCode code, CancellationToken cancellationToken)
        {
            return Revoke(code, cancellationToken);
        }
        ...
        public async Task Authorize(TCode deviceCode, string userId, CancellationToken cancellationToken)
        {
            if (deviceCode == null)
            {
