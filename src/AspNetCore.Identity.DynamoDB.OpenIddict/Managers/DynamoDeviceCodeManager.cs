...
namespace AspNetCore.Identity.DynamoDB.OpenIddict.Managers
{
    public class DeviceCodeManager<TCode> : DeviceCodeManager<TCode, string>
            where TCode : class
    {
        public DeviceCodeManager(IDeviceCodeStore<TCode> deviceCodeStore, IServiceProvider services) : base(deviceCodeStore, services)
        {
        }
    }
